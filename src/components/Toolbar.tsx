      ...
      {isMobile ? (
        ...
        <>
          ...
          <div className="flex items-center gap-2 px-3 flex-shrink-0">
            ...
            <span className="font-bold text-[12px] tracking-wide hidden sm:inline">{t("app.name")}</span>
            <span
          ...
          {price !== null && (
            <div className="flex items-center gap-1.5 font-mono flex-shrink-0">
              ...
              {changePct !== null && (
                <span className={`text-[10px] font-semibold ${isUp ? 'text-[#22c55e]' : 'text-[#ef4444]'}`}>
                  {changeDisplay}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 ml-auto pr-3">
            {/* 横向滚动时间周期 */}
            <div className="flex items-center gap-0.5 overflow-x-auto scrollbar-hide" style={{ maxWidth: 200 }}>
              {TIMEFRAMES.map(tf => (
                <button
                  key={tf.key}
            ...
            <div className="relative" ref={ref}>
              <button
                ...
                disabled={isLoading}
                className="flex items-center gap-0.5 px-2 py-1 rounded bg-[#111827] border border-[#2d3a52] text-[10px]"
              >
                <span style={{ color: asset.color }}>●</span>
                <span className="text-[#94a3b8]">▼</span>
              </button>
              {open && (
                <div className="absolute top-full right-0 mt-2 w-32 bg-[#111827] border border-[#2d3a52] rounded-lg shadow-xl z-50 overflow-hidden">
                  {ASSETS.map(a => (
                    <button
                      key={a.key}
                      ...
                      style={{ color: symbol === a.key ? '#e2e8f0' : '#94a3b8', fontSize: 12 }}
                    >
            ...
            <button
              ...
              className="text-[10px] px-1.5 py-0.5 rounded bg-[#1a2236] text-[#94a3b8] border border-[#1e293b] transition-colors flex-shrink-0"
            >
              {t("toolbar.lang")}
            </button>
